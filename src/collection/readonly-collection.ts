...
export class ReadonlyCollection<TEntity> {
  ...
  select<TKey extends keyof TEntity> (...properties: TKey[]): ReadonlyCollection<{[P in TKey]: TEntity[P]}> {
    return this.query({
...
export interface CollectionConstructor<TEntity=any> {
  new (dataStore: DataStore, query: ListQuery): ReadonlyCollection<TEntity>;
